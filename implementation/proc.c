 ...
 */
  ...
  register struct proc *p; //diergasia poy briskomaste ekeinh thn stigmh
  register struct proc *usage; //gia krathma ths kaluterhs diergasias xrhsth(ayths me to mikrotero fss_priority)
  register struct proc *end; //to telos ths ouras
  int min = 10000;
  for (q=0; q < NR_SCHED_QUEUES; q++) {
	if(q==USER_Q){
		if(rdy_head[q]){
			...
			p = rdy_head[q];
			min = p->p_fss_priority;
			usage = p;
		}else{
			continue;
		}
		while(p!=end){ //mexri na ftasoume sto telos ths ouras
			p = p->p_nextready; //pame sthn epomenh diergasia pou einai etoimh na trexei
			if(p->p_fss_priority<min){
				min = p->p_fss_priority;
				usage = p;
			}
		}
		rp = usage; //h diergasia pou telika tha elegxthei an mporei na trexei
	}else{
		if(!(rp = rdy_head[q])) {
